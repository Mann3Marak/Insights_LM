...
import { useActionItems } from '@/hooks/useActionItems'; // Import useActionItems
...
const NoteEditor = ({ note, onSave, onDelete, onCancel, isLoading, onCitationClick }: NoteEditorProps) => {
  ...
  const { addActionItem, isAdding } = useActionItems(note?.notebook_id); // Initialize useActionItems hook
  ...
  const handleSaveToActionItem = () => {
    if (!note?.notebook_id || !user?.id || !content.trim()) {
      console.error('Cannot save to action item: Missing notebook ID, user ID, or content.');
      return;
    }
    addActionItem({
      notebook_id: note.notebook_id,
      user_id: user.id,
      action_text: content.trim()
    });
  };
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="p-4 border-t border-gray-200 flex-shrink-0">
          <div className="flex justify-between">
            ...
            <Button 
              onClick={handleSaveToActionItem}
              disabled={!note?.notebook_id || !user?.id || !content.trim() || isAdding}
              size="sm"
              variant="outline"
              className="mr-2"
            >
              <Save className="h-4 w-4 mr-2" />
              {isAdding ? 'Saving Action...' : 'Save to Action Item'}
            </Button>
