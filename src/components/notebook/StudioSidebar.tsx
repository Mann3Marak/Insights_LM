...
const StudioSidebar = ({
  ...
  const {
    ...
    isUpdating: isUpdatingActionItem,
    isDeleting: isDeletingActionItem
  ...
  const handleSaveActionItem = (text: string, isCompleted: boolean) => {
    if (editingActionItem) {
      updateActionItem({
        id: editingActionItem.id,
        updates: { action_text: text, is_completed: isCompleted }
      });
    } else {
      if (notebookId) {
        addActionItem({
  ...
  const isEditingActionItemMode = editingActionItem || isCreatingActionItem;
  ...
  if (isEditingActionItemMode) {
    return <div className="w-full bg-gray-50 border-l border-gray-200 flex flex-col h-full overflow-hidden">
        <ActionItemEditor 
          actionItem={editingActionItem || undefined} 
          onSave={handleSaveActionItem} 
          onDelete={editingActionItem ? handleDeleteActionItem : undefined} 
          onCancel={handleCancel} 
          isLoading={isAddingActionItem || isUpdatingActionItem || isDeletingActionItem} 
        />
      </div>;
  }
  ...
  return (
    <div className="w-full bg-gray-50 border-l border-gray-200 flex flex-col h-full overflow-hidden">
      ...
      {/* Notes and Action Items Section */}
      <div className="flex-1 overflow-hidden">
        <ScrollArea className="h-full">
          <div className="p-4">
            <Accordion type="multiple" defaultValue={["notes", "action-items"]} className="w-full">
              {/* Notes Accordion Item */}
              <AccordionItem value="notes" className="border-b">
                <AccordionTrigger className="flex items-center justify-between py-4 text-lg font-medium text-gray-900 hover:no-underline">
                  Notes
                </AccordionTrigger>
              ...
              {/* Action Items Accordion Item */}
              <AccordionItem value="action-items" className="border-b">
                <AccordionTrigger className="flex items-center justify-between py-4 text-lg font-medium text-gray-900 hover:no-underline">
                  Action Items
                </AccordionTrigger>
                <AccordionContent>
                  {/* Content from ActionItemsSidebar */}
                  <ActionItemsSidebar 
                    notebookId={notebookId} 
                    onActionItemClick={handleEditActionItem}
